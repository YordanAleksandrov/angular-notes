...
angular.
  module('content').
	component('contentDiv', {
		...
		controllerAs: 'self',
		controller:['$rootScope','tabSelection','$http','getNotesFactory','$interval','$q', function contentController($rootScope,tabSelection,$http,getNotesFactory,$interval,$q) {
			...
			this.removeNote = function(note,index,searchBox) {
				...
				if(typeof note.message == 'string'){
					...
					getNotesFactory.removeNote(note).then(function(){
						...
						for(var i=0;i<$rootScope.noteDb.length;i++){
							
							if($rootScope.noteDb[i]._id == note._id){
								
								$rootScope.noteDb.splice(index,1);
					...
					getNotesFactory.removeLNote(note).then(function(){
						...
						for(var i=0;i<$rootScope.notelistDb.length;i++){
							
							if($rootScope.notelistDb[i]._id == note._id){
								
								$rootScope.notelistDb.splice(index,1);
			...
			this.filterNotes = function(filter){
				...
				for(var i=0 ;i< $('.note').length; i++){
					...
					var actual = $rootScope.noteDb[i].head.toLowerCase();
			...
			this.filterLNotes = function(filter){
				...
				for(var i=0 ;i< $('.Lnote').length; i++){
					...
					var actual = $rootScope.notelistDb[i].head.toLowerCase();
					var isSubstring = ( actual.indexOf( filter ) !== -1 );
			...
			this.saveEditedNote = function (note,list,parentIndex){
				...
				if(typeof note.message == 'string'){
					...
					var index = $rootScope.noteDb.indexOf(note);
					...
					var index = $rootScope.notelistDb.indexOf(note);
